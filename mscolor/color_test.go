...
var tests = map[string]*Case{
	...
	"#f06b66": &Case{h: 2, s: 0.58, v: 0.94, r: 240, g: 106, b: 102},
	"#7ba444": &Case{h: 86, s: 0.59, v: 0.64, r: 122, g: 164, b: 68},
...
func TestColor_MakeRGB(t *testing.T) {
	for k, src := range tests {
		c := FromHSV(src.h, src.s, src.v)
		c.MakeRGB()

		if math.Pow(float64(int(c.R)-int(src.r)), 2) > 2 {
			t.Errorf("%v: Red expected %v, actual %v", k, src.r, c.R)
		}

		if math.Pow(float64(int(c.G)-int(src.g)), 2) > 2 {
			t.Errorf("%v: Green expected %v, actual %v", k, src.g, c.G)
		}

		if math.Pow(float64(int(c.B)-int(src.b)), 2) > 2 {
			t.Errorf("%v: Blue expected %v, actual %v", k, src.b, c.B)
		}
	}
}
